...
class SurveyForm(forms.ModelForm):
    ...
    def clean_status(self):
        """Ensure republished surveys are valid."""
        status = self.cleaned_data.get('status')
        if status == 'republished':
            if not self.instance or self.instance.status != 'published':
                raise forms.ValidationError("Only published surveys can be republished.")
        return status
    ...
    def __init__(self, *args, **kwargs):
        ...
        if questions:
            for question in questions:
                ...
                elif question.question_type == 'radio':  # Changed 'multiple_choice' to 'radio'
